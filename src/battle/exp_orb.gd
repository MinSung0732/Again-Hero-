...
func _physics_process(delta: float) -> void:
	...
	var speed_scale := 1.0 + clampf((pickup_radius - distance) / maxf(pickup_radius, 1.0), 0.0, 1.0) * 0.45
	global_position += global_position.direction_to(hero.global_position) * attraction_speed * speed_scale * delta

func _draw() -> void:
	var pulse := 1.0 + sin(pulse_time * 6.0) * 0.10
	var radius := 10.0 * pulse
	draw_circle(Vector2.ZERO, radius + 5.0, Color(0.20, 0.85, 1.0, 0.18))
	draw_circle(Vector2.ZERO, radius, Color(0.20, 0.85, 1.0))
	draw_circle(Vector2.ZERO, radius * 0.45, Color(0.82, 0.98, 1.0))
