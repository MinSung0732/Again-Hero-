...
### Stage 5 대마법 용사 — 원소 평타 골격 / 테스터 전체 해금
- Stage 5 테스트 스테이지와 `대마법 용사` 프로필을 등록. Stage 1 견습 마법용사가 성장해 돌아온 콘셉트로 연결.
- Stage 5 기본 공격은 `effect6/effect7`의 orb 프레임만 사용하고 원소별 색상/절차형 피격 피드백으로 구분해, `effect1~5`, `effect8`은 추후 스킬 이펙트용으로 보존.
- 매 평타마다 직전 원소를 제외한 6원소(땅/불/얼음/빛/바람/신성) 중 하나를 새로 선택.
- 땅: 투사체 속도 감소, 직격 피해 강화 + 주변 범위 피해.
- 불: 직격 후 3초 화상 지속 피해.
- 얼음: 적중 시 10% 확률로 1초 빙결(행동 정지).
- 빛: 적중한 적에서 가까운 적 1명으로 추가 피해가 도탄하며 짧은 체인 라이트닝 선을 표시.
- 바람: 투사체 관통 + 지나간 적 넉백.
- 신성: 설정형 확률/회복량으로 자가 치유, `undead/is_undead` 메타 대상에게 추가 피해. 현재 임시 기본값은 15% / 25 회복 / 언데드 x1.5이며 추후 기획값으로 교체 가능.
- Stage 5 증강 풀은 공용 9종만 연결하고 전용 스킬/전용증강은 아직 추가하지 않음.
- 테스터 모드 `TESTER_UNLOCK_ALL_STAGES`를 추가해 StageCatalog에 등록된 모든 스테이지를 자동 해금. 이후 Stage 6+가 추가돼도 별도 해금 숫자 수정 없이 바로 선택 가능.
- Stage 5 스테이지 시간/맵/보상은 테스트 셸 값으로만 등록했으며 이벤트 타임라인은 비워 둠.


### 공통 보물상자 기믹 / Stage 4 전용증강 AI
- 공통 용사 기술 쿨타임 HUD 추가: 전투 화면 좌측 상단에 쿨타임이 존재하는 기술만 62px 아이콘 뱃지로 표시하고, 쿨타임 중 아이콘을 어둡게 비활성화하며 12시 방향부터 시계방향 원형 마스크가 남은 쿨 비율만큼 줄어들도록 구현. 현재 메이지 3개(마력 관통포/마력 장벽/비전 집중), 도적 2개(난도질/급습-암살), 검방 1개(검방 돌격), 거너 3개(백스텝/실린더타격/데드아이)를 공통 데이터 경로로 지원.
- 보물상자 파괴 시 경험치스톤 다발 수량을 6~10개 → 15~20개로 대폭 상향. 각 스톤의 EXP 10~30은 유지.
- 보물상자 임시 체력을 60 → 100으로 상향하고 HP바를 추가해 피격 여부를 시각적으로 확인 가능하게 수정.
- 메이지/거너 원거리 투사체에 이동 선분 기반 상자 보정 판정을 추가해 빠른 탄환이 작은 상자 히트박스를 프레임 사이로 통과하는 경우에도 안정적으로 피해 적용.
- 보물상자를 용사 공격 타깃 탐색에서 몬스터와 동급 후보로 포함해 원거리/근거리 용사가 모두 직접 조준·공격하도록 수정. 단, 실제 `monsters` 그룹에는 넣지 않아 AI 전황 집계와 처치 카운트에는 섞이지 않게 유지.
- Hero가 직접 처치한 몬스터(자폭 제외) 50마리마다 맵 랜덤 위치에 보물상자 생성.
- 보물상자는 필드에 최대 2개까지 존재하며, 2개가 차 있으면 50킬 트리거를 보존해 다음 유효 처치 때 다시 생성 시도.
- 상자 도트 리소스가 준비되기 전까지 코드 기반 임시 상자 비주얼을 사용하며, 별도 TreasureChest 씬으로 분리해 추후 PNG 교체 가능하게 구성.
- 상자 파괴 시 경험치스톤 6~10개가 상자 중심에서 방사형으로 튀어나오며 각 스톤 EXP는 10~30.
- 모든 Hero AI는 기존 전투/회복 행동을 유지하면서 상자 방향을 약 0.22~0.42 가중치로 이동 벡터에 섞어 장기적으로 접근.
- 전투 중 상자를 강제 최우선 타깃으로 바꾸지 않으며, 원거리 투사체와 근접 공격 판정으로 상자를 파괴 가능.
- 주변 몬스터가 없을 때는 회복 아이템을 우선하고, 그 다음 상자를 향해 이동해 근접 시 직접 파괴.
- 권총의 용사 전용증강 15종 추가: 장탄/재장전, 헤드샷, 백스텝/실린더, 데드아이 계열.
- Stage 4 AI 증강 선택에 현재 장탄 부족도, 재장전 여부, 포위 압박도, 데드아이 몬스터 밀집 점수를 전용 관찰값으로 추가.
- 전용증강은 기존 플레이어 소환 타입/역할 비율, 최근 공세, HP, 기존 빌드 시너지와 함께 점수화해 상황별로 선택.
- 거너 전투 루프에서도 AI 관찰값을 주기적으로 갱신하도록 보완.


### Stage 4 권총의 용사 전투 골격
- 거너 버그 수정: 데드아이 중 이동속도에 잘못 적용되던 ×0.35 감속을 제거해 설정값(기본 +30%)대로 이동하도록 수정하고, 실린더 먼지 이펙트 색상을 매 발동마다 회갈색으로 재적용해 2회차부터 흰색으로 표시되던 문제 수정.
- 공통 보스 선택 규칙 통일: Stage 1~4 보스는 특정 몬스터 고정이 아니라 현재 편성팀 몬스터 중 무작위 1종을 선택해 해당 스테이지의 보스 배율을 적용. 실제 선택된 몬스터명으로 보스 명칭을 표시.
- Stage 4 엘리트/미니보스/보스 이벤트 연결: 135초 1차 돌연변이(+증원 8), 270초 2차 돌연변이(+증원 10), 405초 대돌연변이(+증원 14), 480초 편성팀 무작위 보스(+증원 16). 엘리트/미니보스는 기존 편성 몬스터 선택형 돌연변이 시스템을 그대로 사용하며 보스는 HP x8.5 / 공격력 x2.8 / 이동속도 x1.20 / EXP x3.8 / 크기 x1.55로 설정.
- Stage 4 난이도 1차 상향: 권총의 용사 최대 HP 1000→1600, 백스텝 쿨타임 5.5→4.5초로 강화. 공격력 78 및 기존 탄창/재장전/실드 규칙은 유지.
- 권총의 용사 기본 공격력 36 → 78로 추가 상향. 연구 0 슬라임 기본 HP 72 기준으로 비헤드샷 기본 탄환 1발에 처치 가능하도록 조정.
- 거너 전용 맵 경계 복귀 조향 추가: 적 주위를 접선 선회하다 맵 끝에 고착되는 현상을 막기 위해 가장자리 360px 구간에서 안쪽 방향 가중치를 점진적으로 강화. 회복 아이템/보물상자를 직접 추적할 때는 목적지 접근을 우선하며, 데드아이 이동에도 동일한 경계 보정 적용.
- Stage 4 권총의 용사 대폭 상향: HP 920→1000, 이동속도 267→300, 공격력 24→36, 사거리 520→560, 공격 쿨 0.48→0.40, 탄속 920→1000, AI 감지 520→560, 카이팅 거리 260→290, 재장전 2.4→1.8초, 백스텝 무적 0.75→0.90초/거리 260→300/쿨 7→5.5초/기본 발동 30→40%, 실린더 쿨 10→8초/반경 190→210/넉백 145→175, 데드아이 쿨 20→16초.
- 권총의 용사가 탄창을 완전히 소모해 장탄이 0이 되는 순간 최대 체력의 10% 보호막을 즉시 획득. 보호막은 중첩하지 않고 매번 새 10% 값으로 갱신되며, 재장전 완료 시점이 아닌 탄창 소진 시점에 발동. 데드아이의 잔탄 전량 소모도 동일하게 적용.
- 백스텝 탈출 방향을 16→32방향 평가로 세분화하고, 주변 몬스터 가중 중심의 반대 방향·이동 통로 밀도·도착지 근접 몬스터·맵 경계 클램프 손실을 함께 평가해 몬스터 밀집 지역으로 다이브하는 선택을 강하게 억제.
- 신규 전용증강 `화약 가속`: 탄약 1칸 소비마다 공격력 누적 증가. 증강 1레벨 3%, 이후 레벨당 +1%로 최대 6%. 실제 재장전 시작 시 누적 초기화하며 퀵드로 재충전은 초기화하지 않아 연속 퀵드로 시 누적 유지.
- 데드아이 도탄에만 전용 시각효과 추가: 도탄 순간 노란/청백색 스파크와 다음 대상 방향으로 0.12초 짧은 잔광 트레일 표시.
- 권총의 용사 기본 공격력 19 → 24로 상향.
- `도탄 사격`을 기존 관통 후 피해 증가 효과에서 실제 도탄으로 수정: 몬스터 명중 시 반경 260px 내 아직 맞지 않은 가장 가까운 몬스터로 탄환 방향이 꺾이며, 중첩당 도탄 가능 횟수 +1 (최대 5회).
- 백스텝 잔상을 3단 → 4단으로 늘리고 밝기/알파/잔존시간/크기를 강화해 시인성 개선.
- 백스텝 탈출 방향을 16방향으로 평가하고, 종착점 위험도보다 전방 통로의 몬스터 밀도를 더 크게 반영해 빈 공간 쪽을 우선 선택.
- 데드아이 발동 판단을 360도 36방향으로 몬스터 밀집도를 샘플링해 가장 밀집된 방향을 선택하도록 변경.
- 데드아이는 최소 밀집 점수 이상에서 후보가 되며, 밀집도가 높을수록 발동 확률이 상승하고 강한 밀집 구간에서는 즉시 우선 사용.
- 데드아이 난사 방향은 동/서/남/북 고정이 아니라 계산된 임의 360도 벡터를 그대로 사용.
- 백스텝 발동 시 이동 경로에 3단 잔상이 남도록 시각 효과 추가.
- 실린더 타격에 기존 Stage 3 ground impact 프레임을 재활용한 회갈색 먼지 충격파 이펙트 추가.
- 백스텝은 피격 시 기본 발동 확률 30%에 주변 220px 내 몬스터 포위 압박도를 가중해 최대 약 85%까지 상승.
- 실린더 타격은 장전 중 주변 2마리부터 후보가 되며, 4마리 이상 포위 시 최우선 발동. 판단은 0.35초 간격으로 제한.
- 권총의 용사 기본 스탯 조정: HP 380 → 920, 공격력 30 → 19, 이동속도 295 → 267.
- 거너 이동 중 전용 걷기 애니메이션과 실제 이동 방향 기반 좌우 전환을 적용.
- 재장전 중 12칸 노란 장탄 게이지가 장전 진행도에 맞춰 0 → 12칸으로 차오르도록 시각 피드백 추가.
- 거너 투사체 collision mask를 몬스터 레이어(2)에 맞춰 수정해 실제 피격/데미지 판정 복구.
- Stage 4 `effect_projectile_01~13`을 총알 투사체 비주얼로 사용하고, `effect_explosion_01~07`을 총구 화염 이펙트로 연결.
- Stage 4 `권총의 용사` 프로필/AI/스테이지 등록 및 전용 도트 리소스 연결.
- 노란 게이지를 기존 궁극기 게이지 대신 12칸 장탄 표시로 사용.
- 기본 공격 1회당 장탄 1칸 소모, 2발 발사: 조준 방향 1발 + 랜덤 각도 1발.
- 권총 탄환은 기본 관통형이며 Stage 4 projectile 이펙트 리소스 사용.
- 장탄 소진 시 기본 2.4초 재장전. 퀵드로 기본 확률 3%로 공격 시 즉시 완전 장전.
- 헤드샷 임시 기본 확률 10%, 피해 1.2배. 헤드샷 피해 숫자는 빨간색 표시.
- 백스텝은 피격 후 쿨타임 조건으로 발동하며 잠시 무적/충돌무시 후 몬스터 밀도가 낮은 방향으로 이탈.
- 실린더 타격은 장전 중, 쿨타임 10초 조건으로 주변 몬스터를 넉백하고 50% 둔화 적용.
- 데드아이는 쿨타임 20초, 남은 장탄을 모두 소모해 남은 장탄 ×2발 난사하고 사용 중 이동 보너스 적용.
- Stage 4 증강 풀은 공용 증강만 사용하며 전용 증강은 아직 추가하지 않음.
- Stage 4 이벤트 타임라인은 전용 기획 전까지 비워 둠.

### 마왕 마력 시스템 개편
- 인게임 `마왕 필살기` 표기를 `마력`으로 변경.
- 원형 포위 마력 70, 일직선 공세 마력 40, 사각 포위 마력 100 소모.
- 기술 사용 시 게이지 전체 초기화가 아니라 해당 마력 비용만 차감.
- 사각 포위 소환 수를 16 → 20마리로 상향.


### 인게임 체력바 진영 색상 정리
- 플레이어가 소환하는 몬스터 체력바를 빨강 → 초록으로 변경.
- 적 용사 체력바를 초록 → 빨강으로 변경.
- 슬라임 / 오크 / 거미 / 폭탄쥐 공통 적용.


### Hero 공용 화력 증강 / Stage 1 탄환 강화 / Stage 3 베기 숙련
- 모든 Hero 공용 증강 `공격 단련` 추가: 공격력 +2, 최대 20중첩.
- 기존 `연사 강화`를 공용 `속공 훈련`으로 개편: 공격속도 +2%, 최대 20중첩(최대 +40%).
- 공용 공격속도는 견습 마법용사 평타, 도적 기본 연격 간격, 검방용사 기본 공격/베기 연격에 공통 적용.
- Stage 1 `탄환 강화`를 투사체 공격력 +8에서 기본 공격 투사체 +1로 변경.
- 탄환 강화 최대 4중첩, 기본 1발에서 최대 5발까지 12도 간격 부채꼴 발사.
- Stage 3 `방패 행군` 증강을 카탈로그/후보 풀/AI 선호도에서 삭제.
- Stage 3 `베기 숙련` 추가: 베기 발생 시 중첩당 추가 1타, 최대 2중첩으로 총 3연격.
- 베기 숙련 추가타는 추가 공격을 다시 발동하지 않으며, 추가타 처치 시 적 1마리당 HP 10 고정 회복.
- 베기 연격 프레임 방향은 1타 정방향 → 2타 역방향 → 3타 정방향으로 재생.


### Stage 3 검방용사 — 막기 조건 / 체력 상향
- 검방용사 초기 최대 HP를 650 → 1100으로 상향.
- 막기 게이지가 가득 차도 주변 320px 안에 몬스터가 없으면 즉시 발동하지 않도록 변경.
- 게이지는 100% 상태로 대기하며, 주변에 몬스터가 1마리 이상 진입하면 막기를 발동.
- 막기 발동 조건은 Hero ultimate 설정값(`activation_enemy_radius`, `activation_enemy_count`)으로 조정 가능하게 구성.


### Stage 보상 구조 조정
- 최초 클리어 연구 포인트를 Stage 1/2/3 기준 250 / 400 / 600으로 하향.
- 성공 클리어 시 Run 연구 보상에 Stage 배율 적용: Stage 1 ×1.00 / Stage 2 ×1.35 / Stage 3 ×1.70.
- 실패/시간 초과 Run은 기존 기본 연구 보상을 유지해 고스테이지 보너스 악용을 방지.
- 로비 스테이지 카드에 최초 클리어 보상과 반복 클리어 배율을 함께 표시.


### Hero 생존 AI — 회복약 판단/접근
- 모든 Hero가 HP 상황에 따라 전장 `heal_items`를 Utility 목표로 평가하도록 추가.
- HP 60% 이상에서는 회복약을 보존하고, 40~60%는 가까운 회복약만 고려, 40% 미만부터 접근 우선도를 단계적으로 높인다.
- 회복약 접근은 강제 직선 이동이 아니라 기존 전투 이동 + 회복약 방향 + 몬스터 군집 회피를 합성한다.
- 약 0.15~0.30초 간격으로 회복약 목표를 재평가하며 위험한 경로는 우회하거나 포기한다.
- 회복약을 노리는 중에도 기존 공격/방어/돌진/암살 행동은 유지한다.
- Hero AI 프로필에 `heal_item_desire`, `heal_risk_tolerance`, `heal_detour_weight` 성향값을 추가했다.

### Stage 3 검방용사 — 승전의 호흡 밸런스
- 최대 중첩을 1 → 10으로 변경.
- 1중첩은 검방 돌격 처치 시 HP 8 회복.
- 이후 중첩당 회복량 +2, 10중첩 최대 HP 26 회복.

### 회복약 필드 누적 제한
- 기존 30킬마다 생성 규칙은 유지.
- 필드에 동시에 존재할 수 있는 회복약을 최대 2개로 제한.
- 최대치에 도달하면 추가 생성은 보류하고 킬 트리거는 30에서 대기.
- 회복약이 소비된 뒤 다음 유효 처치가 발생하면 대기 중인 트리거로 다시 생성.
...
### Stage Meta Final Nudge + Stage 1 Prev Arrow Hide
- STAGE 번호/침입자 제목을 기존 92px StageMetaBox 안에서 4px 더 아래로 이동해 침입자 기록 영역과 카드 상단 테두리 사이 여백 안쪽에 더 안정적으로 배치.
- StageCard/외곽 프레임/헤더 높이는 변경하지 않음.
...
### Section Header Extra 8px Down
- '침입자 기록'과 '대상 용사를 선택하고 던전에 입장하세요.' 두 줄만 추가로 8px 하향.
- STAGE 번호/침입자 제목, StageCard, 초상화, 하단 정보, 스와이프/드래그 및 전환 애니메이션은 변경하지 않음.

### Section Header Another 8px Down
- '침입자 기록'과 '대상 용사를 선택하고 던전에 입장하세요.' 두 줄만 추가로 8px 더 하향.
- STAGE 메타, 카드, 초상화, 하단 정보, 스와이프/드래그, 전환 애니메이션은 변경하지 않음.

### PC Header Vertical Finish Pass 1
- 중앙 Content/StageCard의 시작 Y를 고정한 채 Header만 세로로 20px 확장.
- SafeArea top을 20→0, Header 높이를 192→212로 동시에 조정해 Header 하단 좌표는 기존과 동일(212)하게 유지.
...
### Stage 1 Mage Remake Scale Fix
- Stage 1 법사 리메이크 프레임 적용 후 전투 화면에서 캐릭터가 비정상적으로 크게 표시되는 문제 수정.
- 원인: 새 본체 프레임은 256x256 개별 PNG인데, 스케일 정규화 기준은 여전히 구형 64x64 통합 시트의 첫 프레임을 사용하고 있었음.
- 구형 시트 기준 높이를 얻지 못하면 scale=3.0을 그대로 적용하는 fallback 때문에 256x256 리메이크 프레임이 과대 렌더링될 수 있었음.
- Stage 1 법사는 실제 alpha used rect 높이를 기준으로 목표 화면 높이 108px에 자동 정규화하도록 변경.
- Stage 2 이후 영웅도 구형 기준 시트를 읽지 못하는 경우 같은 안전 목표 높이를 사용하도록 fallback 개선.

### Stage 2 Rogue Augment Rebalance + 4-Hit Upgrade
- 그림자 도적 기본 체급 소폭 상향: HP 440→460, 공격력 38→42, 이동속도 270→275.
- 대신 기본 연격 속도는 하향: 초반 연타 간격을 0.24/0.26/0.28초로 늦추고 마무리 후딜 1.05초 적용.
- 기본 3연타 피해 배율을 85%/100%/135%에서 50%/50%/70%로 조정. 4타 확장 시 추가타도 70%.
- 예리한 연격: 곱연산(1.1^N) 제거, 중첩당 +10% 합연산 / 최대 +50%.
- 연격 가속: 마무리 후딜 중첩당 -3%, 최대 4중첩(-12%).
- 난도질 방어: 중첩당 최대 HP +4%→+2%, 최대 +8%.
- 끈질긴 급습: 최대 2→4중첩, 급습-암살 기본 5타에 최대 +4타.
- 처형 감각: 기본 처형 기준 30%→10%, 중첩당 +2% 유지, 최대 3중첩으로 총 16%까지.
- 피의 갈증: 3%/5% 두 단계로 축소하고 최대 흡혈을 5%로 제한.
- 신규 증강 '무자비한 일격' 추가: 최대 1중첩, 기본 연격을 3타 돌진에서 4타 돌진으로 확장.
- 도적 콤보 길이를 런타임에서 3+추가타로 계산하도록 변경해 향후 추가타 계열 증강 확장 가능.

### Common Hero Growth Augments
- 모든 현재 용사가 공유하는 성장 계열 증강 2종 추가.
- 경험 축적: 경험치 획득량 +10% / 최대 5중첩 / 최대 +50%.
- 경험 흡수: 경험치 획득 범위 +40 / 최대 4중첩 / 총 +160.
- Hero에 exp_gain_multiplier 런타임 스탯을 추가하고 gain_exp()에서 실제 획득 경험치에 적용.
- 견습 마도사와 그림자 도적의 augment_pool_ids에 두 증강을 모두 등록.
- 현재 두 용사가 공통으로 공유하던 기존 증강은 민첩한 발놀림(pursuit), 둔화 적응(slow_resistance) 2종이었음.

### Common Max HP Augment
- 기존 견습 마도사 전용 풀에만 있던 '강인한 육체'를 공통 증강으로 확장.
- 그림자 도적 augment_pool_ids에도 iron_body를 추가해 모든 현재 용사가 최대 HP 증가 증강을 선택 가능.
- 효과는 기존과 동일: 최대 HP +45 및 즉시 HP +45 회복, 최대 5중첩.
- 현재 공통 증강은 강인한 육체 / 민첩한 발놀림 / 둔화 적응 / 경험 축적 / 경험 흡수 총 5종.

### Common Hero Augment Long-Run Rebalance
- 공통 증강이 너무 빨리 최대 중첩에 도달해 후반 레벨업 선택지가 사라지는 문제를 완화.
- 모든 공통 증강 최대 중첩을 최소 10레벨로 통일하고, 1회 선택당 증가량을 낮춰 성장 곡선을 길게 조정.
- 강인한 육체: 최대 5→10중첩, 최대 HP/즉시 회복 +45→+20 (최대 +200 HP).
- 민첩한 발놀림: 최대 2→10중첩, 이동속도 +25→+8 (최대 +80).
- 둔화 적응: 최대 3→10중첩, 중첩당 +18%→+5%, 총 저항 상한 50%.
- 경험 축적: 최대 5→10중첩, 경험치 +10%→+4% (최대 +40%).
- 경험 흡수: 최대 4→10중첩, 획득 범위 +40→+15 (최대 +150).
- 전용 증강의 중첩 구조는 이번 패스에서 유지.

### Stage 2 Rogue Bloodthirst Scaling
- 피의 갈증을 장기 성장형 증강으로 변경.
- 중첩당 직접 피해 흡혈 +2%.
- 최대 10중첩, 최대 흡혈 20%.
- 기존 3%/5% 단계식 전용 연산을 제거하고 일반 add_stat 합연산으로 통일.

### Hero Display Name Standardization
- 용사 이름 규칙을 '~용사' 형태로 통일하기 시작.
- Stage 1: '견습 마도사' → '견습 마법용사'.
- Stage 2: '그림자 도적' → '날렵한 용사'.
- Stage 3 신규 용사명은 '검과 방패의 용사'로 확정.
- Stage 3 실제 전투 등록/AI/스킬 구현은 별도 단계에서 진행.

### Stage 3 Sword & Shield Hero
- Stage 3 '검과 방패의 용사' 추가 및 stage_3를 StageCatalog에 등록.
- 기본 스탯: HP 650, 이동속도 200, 공격력 48, 공격주기 0.92초. 느리고 단단한 전선 유지형 근접 용사.
- 기본 공격은 상황형 베기/찌르기. 근처 적이 여럿이면 넓은 베기(effect2), 그렇지 않으면 긴 직선 찌르기(effect3)를 사용.
- 막기 게이지는 공격/처치/피격과 무관하게 고정 시간으로만 충전되며 기본 20초에 완충.
- 막기 발동 시 10초 동안 최대 HP 60% 쉴드, 받는 피해 30% 감소, 이동속도 감소, effect4 오라 지속.
- 막기 중 피해 감소 전 원본 피해를 누적하고 종료 시 effect1과 함께 누적 피해의 50%를 주변 250 범위에 광역 반격.
- 반사 피해는 기본 능력에서 제거. '가시 방패' 증강을 선택해야만 막기 중 원본 피해 일부를 공격자에게 반사.
- Stage 3 전용 증강: 견고한 방패, 복수의 방패, 가시 방패, 수호 숙련, 방패 행군, 방패 충전, 숙련된 검사, 넓은 베기, 관통 찌르기.
- 공통 증강 5종(강인한 육체/민첩한 발놀림/둔화 적응/경험 축적/경험 흡수)도 Stage 3 풀에 포함.
- Stage 3도 기존 HeroBuildAI를 사용해 전장 상황, HP, 적 밀도/역할, 기존 빌드 시너지를 기준으로 증강을 선택.

### Stage 3 Shield Charge
- Stage 3 effect5 (ground_effect 01~08)를 검방 돌격 착지/방패 타격 효과로 연결.
- 검방 돌격은 주변 245 범위에 적 4명 이상일 때 발동하며, 17초 쿨타임.
- 현재 위치에서 최대 560 거리 내 가장 먼 적을 선택해 찌르기 모션으로 초고속 돌진.
- 돌진 중 0.035초 간격으로 용사 본체 잔상을 생성하고, 각 잔상은 0.30초 동안 점점 옅어지며 사라짐.
- 도착 시 effect5 방패 충격과 175 범위 광역 피해를 적용.
- 한 번의 시전에서 최대 3연속 돌진하며 매 타격 후 다음 가장 먼 적을 다시 선택.
- 신규 전용 증강 '용기백배': 1중첩 +15%, 이후 중첩당 +3%, 최대 8중첩 +36%로 검방 돌격 피해 강화.

### Permanent Research Diminishing Returns
- 군단 연구 전반의 초중반 효율은 유지하되 고레벨 구간부터 효과가 점차 감소하도록 구간별 효율을 적용.
- 화력/생체 강화: Lv.1~10 100%, 11~25 70%, 26~45 45%, 46~70 25%.
- 기동/공격 훈련: Lv.1~10 100%, 11~25 60%, 26~45 35%, 46~70 20%.
- 소환 효율화/마력 순환/마력 저장고/고속 실험법도 각 최대 레벨에 맞춘 구간별 감소를 적용.
- 연구 비용도 전반적으로 상향. 전술 기록 노트는 1500 / 4500 / 8000으로 조정.
- 전술 기록 노트는 효과가 정수형 새로고침 횟수라 효과 감소 대신 가격 상승만 적용.

### Stage 3 Fighter Sustain
- 검과 방패의 용사 막기 종료 시 누적한 원본 피해의 12%만큼 HP를 회복.
- 한 번의 막기에서 회복 가능한 양은 최대 HP의 8%로 제한해 유지력은 보완하되 무한 버티기는 방지.

### Shared Hero Healing Item
- 모든 용사/스테이지 공용 회복 아이템 기믹 추가.
- 용사가 몬스터를 30마리 처치할 때마다 맵의 랜덤 위치에 회복 아이템 1개 생성. 폭탄쥐의 자폭 사망은 처치 수에서 제외.
- 새 에셋 assets/art/heroes/item/heal_frames/float_01~04.png를 4프레임 부유 애니메이션으로 사용.
- 용사가 아이템에 닿으면 기본 HP 70 회복.
- 공용 증강 '회복 증폭' 추가: 회복 아이템 효율 +3%/중첩, 최대 20중첩(+60%). 기본 70 → 최대 112 회복.
- Stage 1~3 모든 현재 용사 증강 풀에 회복 증폭을 포함.

### Stage 3 Fighter Charge / Sustain Rework
- 검방 돌격은 용기백배 없이도 기본 3연속 돌진으로 유지.
- 용기백배는 기본 3회 이후 추가 돌진 확률만 부여: 1중첩 15%, 이후 중첩당 +3%, 최대 36%.
- 추가 돌진은 성공할 때마다 다시 확률을 판정하며 한 번의 시전에서 최대 6회까지만 가능.
- 막기 종료 시 누적 피해 기반 HP 회복 효과를 제거해 막기 자체의 피흡/회복을 삭제.
- 신규 검방 전용 증강 '승전의 호흡' 추가: 검방 돌격으로 적 처치 시 고정 HP 8 회복. 최대 1중첩.
- 용기백배의 돌격 처치 회복 효과는 제거하고 연속 돌진 확률 강화 역할로 분리.
