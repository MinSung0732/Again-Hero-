...
## 2026-09-27 — Stage 7 연금술사 가스 고갈 AI 이동 수정
- 가스가 기본 평타 비용보다 부족할 때 재료 방향으로 직선 이동하고 즉시 전투 이동 로직을 종료하던 구조를 수정.
- 주변 몬스터가 카이팅 거리 안에 있으면 재료 추적 방향과 기존 몬스터 회피 방향을 합성해, 포위 상황에서도 재료를 향하면서 탈출 경로를 만들도록 변경.
- 가스 회복 재료 탐색에 일반 필드 재료뿐 아니라 솥 성공 보상 재료도 포함하고, 포물선 비행 중인 재료는 착지 전까지 추적 대상에서 제외.

## 2026-09-26 — 공용 레벨업 SFX 추가 볼륨 하향
- 레벨업 효과음이 연금술사 평타보다 여전히 크게 들리는 점을 반영해 -14dB → -18dB로 추가 하향.
- 평타와 동일한 dB 수치여도 원본 레벨업 음원의 체감 피크가 더 큰 점을 고려해 별도 보정값을 적용.

## 2026-09-26 — 공용 레벨업 SFX 볼륨 조정
- 공용 레벨업 효과음을 -10dB → -14dB로 낮춤.
- Stage 7 연금술사 기본 평타 투척음(-14dB)을 기준으로 체감 음량을 맞춰 레벨업 시 사운드가 과도하게 튀지 않도록 조정.

## 2026-09-26 — Stage 7 연금술사 솥 결과 연출 / 재료 포물선 개선
- 2스킬 솥이 혼합 완료 직후 사라지지 않도록 결과 프레임 4→5→6을 끝까지 표시한 뒤 결과 처리를 시작.
- 대성공/성공/실패 결과 연출이 진행되는 동안 솥을 유지하고, 해당 연출이 끝난 뒤에만 솥을 비활성화하도록 변경.
- 성공 보상 연금술 재료는 솥 중심에서 주변 착지 지점으로 0.36~0.50초 포물선 비행 후 떨어지도록 연출 추가.
- 비행 중 재료는 즉시 획득되지 않도록 막고, 착지 후 기존 가스 획득 규칙을 그대로 사용.

## 2026-09-26 — Stage 7 독장판 착탄 SFX 추가 볼륨 조정
- 기본 공격 약병이 깨지며 독장판이 생성될 때 들리는 파손/착탄 SFX를 -11dB → -15dB로 낮춤.
- 3개 약병이 연속/중첩 착탄할 때 체감 음량이 과도하게 커지는 현상을 완화하고, 투척음(-14dB)과 비슷한 기준으로 정리.

## 2026-09-26 — Stage 7 연금술사 SFX 볼륨 정규화
- 평타 약병 SFX(투척 -14dB / 파손 -11dB)를 기준으로 솥 결과음의 체감 음량 차이를 축소.
- 대성공 효과음을 -8dB → -12dB로 낮춰 평타 대비 과도하게 튀는 피크를 완화.
- 성공/실패 효과음도 -10dB → -12dB로 맞춰 연금술 결과음 전체 레벨을 일관되게 정리.
- 솥 설치음(-13dB)은 기존 값을 유지.

## 2026-09-26 — Stage 7 연금술사 SFX OGG 긴급 복구
- 직전 현대식 SFX 교체에서 추가된 OGG 파일이 일부 환경에서 정상 디코딩/재생되지 않는 문제를 확인.
- 인게임 무음 문제를 즉시 막기 위해 마지막으로 정상 동작하던 연금술사 WAV 6종을 복구하고 솥/약병 씬 참조도 WAV로 되돌림.
- 문제가 있던 신규 OGG 6종은 저장소에서 제거. 현대식 SFX는 검증 가능한 정상 오디오 파일로 다시 교체하기 전까지 WAV 경로를 유지.
...
- 이동속도 30% 버프/회복 등 실제 장판 효과 판정은 기존 실제 범위를 그대로 사용하고, 애니메이션 전환 상태만 별도로 안정화.
- Stage 7 이동 모션은 안정화된 상태값을 기준으로 walk/run을 선택해 공통 애니메이션 우선순위 시스템과 충돌하지 않도록 변경.


## 2026-09-25 — 공통 용사 애니메이션 중복 재시작/우선순위 안정화
- HeroSprite 공통 애니메이션 요청에 우선순위를 추가: death > hit > dash > attack > run/move > idle.
- 상위 모션이 보호되는 동안 이동/대기 같은 하위 모션이 덮어쓰지 못하도록 공통 재생 함수에서 차단.
- 동일 애니메이션 restart 요청이 70ms 안에 연속으로 들어올 경우 두 번째부터는 프레임 0 재시작만 억제해 부들부들 떠는 현상을 완화.
- 중복 억제는 비주얼 재생에만 적용하고 공격/피해/스킬 판정 로직은 그대로 실행되도록 분리해 실제 행동이 씹히지 않도록 유지.
- 정상적인 다음 공격은 70ms 이후 다시 restart 가능하며, hit/death 같은 상위 상태는 즉시 기존 모션을 끊고 재생 가능.


## 2026-09-25 — Stage 7 촉매 혼합장 생성 애니메이션 추가
- 장판 생성 시 반투명 녹색 내부 영역이 약 0.32초 동안 중앙에서 바깥쪽으로 퍼져나가며 차오르도록 연출 추가.
- 확산 중 선두 가장자리에 옅은 녹색 링을 표시해 스며드는 느낌을 보강.
- 최종 초록색 범위 경계선은 시전 즉시 전체 반경으로 유지해 실제 스킬 범위를 바로 읽을 수 있게 함.
- effect4 연기는 내부 영역이 약 45% 이상 퍼진 뒤부터 생성되도록 해 장판 생성 흐름과 자연스럽게 연결.
- 별도 노드/트윈 생성 없이 기존 장판 노드의 진행값과 draw 갱신만 사용.


## 2026-09-25 — 공통 픽셀아트 떨림 완화
- 모든 용사가 공유하는 Hero Camera2D의 position smoothing을 씬/런타임 양쪽에서 비활성화.
- Godot 2D transform pixel snapping을 활성화해 소수점 이동 좌표가 nearest 픽셀 렌더링에서 프레임마다 흔들려 보이는 현상을 줄이도록 조정.
- 물리 이동 좌표 자체는 연속값을 유지하고 렌더링 정렬만 안정화.


## 2026-09-25 — Stage 7 촉매 혼합장 시각/범위 조정
- 「촉매 혼합장」 반경을 660 → 450으로 축소.
- 장판 내부에 희미한 녹색 반투명 면을 추가하고 기존 초록색 경계선은 유지.
- effect4 연기 풀을 8 → 12개, 생성 간격을 0.18초 → 0.11초, 스케일을 0.55 → 0.62로 조정해 영역이 더 꽉 차 보이도록 개선.
- 연기 무작위 배치 반경을 장판의 94%까지 사용하도록 확대.


## 2026-09-25 — Stage 7 스킬 1 「촉매 혼합장」 추가
- 화학의 용사 기본 이동 애니메이션을 run 프레임에서 walk 프레임으로 변경하고, 스킬 영역 안에서 이동할 때만 run 프레임을 사용.
- 스킬 1 「촉매 혼합장」 추가: 화학가스 30 소모 / 쿨타임 20초 / 반경 660의 고정 장판을 현재 위치에 생성.
- 장판 경계는 초록색 원형 라인으로 표시하고, effect4(effect_13~20) 연기를 고정 풀에서 무작위 위치에 반복 재생해 끓어오르는 느낌을 표현.
- 장판 안 몬스터는 0.5초마다 공격력 20%의 지속 피해를 받고 이동속도 20% 둔화가 갱신됨.
- 장판 안 용사는 이동속도 30% 증가, run 애니메이션 전환, 초당 최대 HP 1% 회복.
- 지속시간은 밸런스 임시값 8초, 자동 사용 조건은 반경 안 몬스터 2마리 이상으로 설정.
- 전용 장판 노드는 런타임 재생성 없이 1회 생성하고 effect4 연기도 8개 풀로 재사용하여 모바일 전투 중 생성/해제 비용을 억제.


## 2026-09-24 — Stage 7 연금술사 상자 직격 처리
- 화학의 용사가 보물상자를 공격 대상으로 삼을 수 있도록 기본 공격의 첫 번째 약병을 상자 중심에 확정 투척하도록 추가.
- 첫 약병 착탄 시 상자에 현재 공격력 기준 1회 직접 피해를 적용하고, 같은 위치에 독가스 장판은 기존처럼 생성.
- 나머지 약병은 기존 랜덤 투척을 유지하며 독가스 지속 피해는 몬스터에게만 적용.
- 전투 중에는 공격 사거리 안의 상자가 현재 몬스터보다 가까울 때 첫 약병을 상자에 배정하고, 몬스터가 없을 때는 상자 사거리까지 접근해 정상 가스 소모/공격 간격을 사용해 투척.


## 2026-09-24 — Stage 7 독가스 범위/이펙트 및 기본 공격 간격 조정
- 화학의 용사 기본 공격 간격을 1.65초 → 2.30초로 변경.
- 독가스 피해 반경을 275 → 206.25로 약 25% 축소.
- 독가스 이펙트 스케일을 1.05 → 0.79로 약 25% 축소.


## 2026-09-24 — 전투 용사 정보창 실시간 증강 스탯 반영
- 전투 중 용사 정보창의 스탯 표시를 프로필 원본값이 아니라 현재 Run의 레벨업/증강이 적용된 런타임 값 기준으로 통일.
- HP / 공격력 / 이동속도 / 공격 사거리 / 투사체 속도 계열은 현재 런타임 스탯을 사용하고, 공격 간격은 공속 증강까지 반영한 실제 공용 공격 간격 계산값을 사용.
- 향후 용사 도감은 HeroProfiles 원본 데이터를 직접 사용하도록 전투 정보창용 런타임 스탯 API를 별도 분리.


## 2026-09-24 — Stage 7 위치/공격 프레임 조정
- 실기기 스크린샷 기준으로 화학의 용사 스프라이트를 체력/가스 바 아래쪽에 더 자연스럽게 맞추도록 Y offset을 -24 → +16으로 조정.
- Stage 7 기본 공격 애니메이션 순서를 atk_01 → atk_02 → atk_05로 변경.
- 좌우 기준 보정값 ±104는 유지.


## 2026-09-24 — Stage 7 캐릭터 중심 미세 보정 2차
- 실기기 스크린샷 기준으로 화학의 용사 중심을 추가 미세 조정.
- Stage 7 전용 X offset을 좌우 ±96 → ±104, Y offset을 -48 → -24로 변경.


## 2026-09-24 — Stage 7 캐릭터 중심 추가 보정
- 실기기 스크린샷 기준으로 화학의 용사 스프라이트가 전투 기준 원보다 위에 떠 보이는 문제를 추가 보정.
- Stage 7 전용 Y offset을 -96 → -48로 조정하고 X 보정(좌우 ±96)은 유지.


## 2026-09-24 — Stage 7 도트 앵커 / 약병 파손 이펙트 보정
- Stage 7 화학의 용사 독립 프레임의 공통 루트 앵커(160,224)를 화면상 Hero 좌표에 맞추도록 전용 sprite offset 보정 추가.
- 좌우 반전 시에도 루트 위치가 흔들리지 않도록 flip_h 상태에 따라 X offset을 ±96으로 보정.
- 프로필 전환 시 다른 용사에 Stage 7 offset이 남지 않도록 sprite offset 초기화 추가.
- 약병 파손 effect5 스케일을 0.58 → 0.34로 축소해 비행 중 약병 대비 과도하게 크게 보이던 현상 완화.


## 2026-09-24 — Stage 7 화학의 용사 기본 전투 골격
- Stage 7 화학의 용사를 추가하고 로비/스테이지 진행을 Stage 7까지 연결.
- 화학가스 최대 200, 기본 독극물 투척 1회당 가스 10 소모. 게이지는 기존 노란색 대신 보라색으로 표시.
- 기본 공격은 atk_01 → atk_03 → atk_04를 사용하고, 0.19초 간격으로 반경 400 내 무작위 위치에 약병 3개를 순차 투척.
- 약병은 effect1/effect_01을 비행 프레임으로 사용하고 착지 시 effect5 파손 효과 후 4초간 반경 275 독장판 생성. 독장판은 effect1 후속 프레임을 사용하며 0.27초마다 피해 판정.
- 연금술 재료는 effect2를 사용해 용사 중심 최대 1500 범위에 최대 10개 유지. 시작 6개, 일반 4초/가스 30% 이하 2초/가스 고갈 상태에서 재료 3개 미만이면 0.8초 간격으로 보충해 자원 고갈 하드락 방지.
- 재료 1개 획득 시 화학가스 20 회복. 가스가 기본 공격 비용 미만이면 AI가 가장 가까운 재료를 우선 회수.
- 약병 8개/독장판 16개/재료 최대 10개를 고정 풀로 생성해 전투 중 instantiate/free 반복을 피함.
- 전용 스킬/전용 증강은 후속 기획 단계에서 추가 예정. 현재 공격력/체력/독 틱 피해 등은 1차 밸런스 전 임시값.
...
### Stage 5 체인대거 잔상 / 벽 코너 AI 수정
- 체인대거 도탄 전류선의 삭제를 투사체 코루틴 종료에 의존하지 않도록 변경. 전류선 자체 Tween이 틱 지속시간 이후 반드시 페이드아웃/queue_free 되어 바닥에 영구 잔상이 남지 않도록 수정.
- 대마법 용사 전용 맵 경계 조향 추가. 벽 440px 이내에서 안쪽 가중치를 주고, 가로/세로 벽이 동시에 가까운 코너에서는 맵 중앙 방향을 강하게 섞어 구석에 계속 밀착되는 현상 완화.
- 기존 공격/회피/회복 아이템/상자 판단은 유지하고 최종 이동 벡터에만 경계 탈출 보정을 적용.


### Stage 5 대마법 용사 기본 체급 2차 상향
- 연소: 화염구 범위 95→123.5(약 1.3배), 시각 크기 약 1.3배, 유지시간 1.8→2.1초.
- 아이스볼트: 항상 마법사 기준 가장 먼 몬스터를 조준. 투사체 속도 1050→700, 사거리 850→1100, 얼음기둥 6→10개.
- 땅의 가시: 피해 배율 기본공격력 120%→140%.
- 신성력: 구체당 타격 반경 86→111.8(약 1.3배), 구체 시각 크기도 같은 비율로 확대, 폭발 7→12개, 생성 간격 0.09→0.12초.
- 체인대거: 투사체 속도 1200→900, 도탄 탐색범위 420→600, 도탄당 피해 증가 18%→20%. 도탄 시 투사체가 새 대상 방향을 바라보도록 갱신. 이전 적과 다음 도탄 대상 사이에 전류선을 남겨 기본공격력 11% 피해를 0.18초 간격으로 4회 적용.
- 조화: 쿨타임 100초는 유지하되 AI 선택 가중치를 크게 높여 준비됐을 때 더 주기적으로 사용. 사용 직후 노란 게이지 50 환급.
- 폭풍: 투사체 본체 시각 크기 약 28% 확대, 꼬리 이펙트 약 32% 축소. 투사체 속도 900→350, 사거리 950→700. 각 8방향 투사체가 종착점에 도달하면 반경 125에서 기본공격력 90% 폭발 피해.
- 전용증강은 여전히 추가하지 않음.


- Stage 5 대마법 용사 기본 공격 조정: 공격력 90→118, 기본 공격 간격 0.58초→1.14초.

### Stage 5/전투 VFX 가시성·성능 조정
- 용사에게 남아 있던 기존 보라색 원형 둔화 표시를 제거하고 신규 slow_01~06 프레임 이펙트만 사용하도록 정리.
- 오크 광폭화 rage_01~08 이펙트를 발 아래에서 몸쪽으로 약 46px 올려 배치.
- 공용 상태이상 이펙트 SpriteFrames를 정적 캐시로 재사용하도록 변경. 몬스터 소환 때마다 slow PNG 6장을 다시 읽던 비용 제거.
- 대마법 용사 평타 orb, 액티브 스킬 FX, 스킬 투사체 SpriteFrames를 캐시하고 ResourceLoader 우선 로드로 변경해 반복 스킬 사용 시 PNG 디스크 재로딩/프레임 재조립 감소.
- 대마법 용사 기술 이펙트 및 투사체/꼬리 표시 크기를 전반적으로 확대해 전투 중 가시성 강화.
- 조화를 제외한 Stage 5 액티브 6종 쿨타임 약 25% 증가: 연소 22.5초, 아이스볼트 25초, 땅의 가시 27.5초, 신성력 30초, 체인대거 25초, 폭풍 32.5초. 조화는 100초 유지.
- 연소 화염구 유지시간을 0.9초에서 1.8초로 증가.


### 공용 상태이상/버프 이펙트 연결
- 신규 공용 슬로우 이펙트 `assets/art/effects/debuff/frames/slow_01~06.png`를 용사 및 현재 몬스터 4종에 연결. 용사의 기존 slow_timer, 거너 실린더/대마법 신성력의 gunner_slow 메타, 대마법 폭풍의 2초 속박 상태에서 자동 표시.
- 신규 오크 광폭화 이펙트 `assets/art/effects/buff/frames/orc_rage/rage_01~08.png`를 `orc_berserk` 특수증강의 실제 발동 조건(HP 50% 이하)과 연결. 광폭화가 활성화된 동안에만 루프 표시.
- 시각효과는 공용 `combat_status_effect_visual.gd` 컴포넌트로 분리해 이후 추가 몬스터/슬로우 기술에도 재사용 가능하도록 구성.


### Stage 5 대마법 용사 — 7개 기술 / 원소 패시브
- 대마법 용사 전용 액티브 7종 구현. 모든 기술은 노란 게이지가 최대치일 때만 발동하며, 개별 쿨타임이 준비된 후보 중 전황 가중치 + 랜덤 선택으로 AI가 사용.
- 연소: effect2 01~07 화염구를 전방 고정 위치에 남겨 이동 중에도 다단히트, 지속 종료 후 적 방향으로 effect2 08~12 연소 돌진 피해.
- 아이스볼트: effect4 08~11 투사체, 적중 시 effect4 07 히트 후 주변 무작위 위치에 effect4 01~06 얼음기둥 다수 생성.
- 땅의 가시: effect1 01~11을 전방 직선상에 빠르게 순차 생성. 웨이브 관통형이며 한 대상은 한 번만 피해.
- 신성력: 주변 무작위 위치에 effect3 01~05를 순차 폭발. 피격 적 이동속도 40% 감소, undead/is_undead 대상 추가 피해 지원.
- 체인대거: effect5 08~11 투사체 + effect5 01~06 명중 이펙트. 최초 명중 후 최대 7회 추가 도탄, 도탄마다 피해 증가. 투사체가 필드에 남아 있는 동안 재사용 불가.
- 조화: effect7 08~12 사용. 쿨타임 100초, 조화를 제외한 나머지 6개 기술 쿨타임 즉시 초기화.
- 폭풍: effect8 01~06 투사체를 8방향 발사하고 effect8 07~09 꼬리를 부착. 관통하며 적중 시 노란 게이지 일부 회복, 현재 몬스터 4종에 2초 이동속도 0% 속박 적용.
- 원소 패시브: 스킬 사용 시 effect6 단일 구체를 몸 주변에 회전 표시. 연소=불01, 아이스볼트=물02, 폭풍=바람03, 체인대거=전기04, 땅의 가시=땅05, 신성력=신성07.
- 6원소가 모두 모인 상태에서 다음 비-조화 기술을 사용하면 구체 6개를 전부 소모하고 해당 기술을 강화. 현 단계 강화 수치는 공통 피해 x1.50 임시값으로 데이터화.
- 전용증강은 추가하지 않았으며 기존 공용증강 9종만 유지.
- 미확정 n 수치(각 기술 피해량/기둥 수/게이지 회복량/쿨타임 등)는 hero_profiles.gd의 archmage_skills 데이터로 분리해 추후 밸런스 회의에서 즉시 조정 가능.
